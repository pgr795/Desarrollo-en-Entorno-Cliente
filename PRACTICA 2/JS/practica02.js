...
// --------------------------------------------------------------------------------------------------------
	...
	function anadirDefinicion(){
		let padre=$("#definicion");									//document.querySelector("#definicion");
		let vPalabra=$("#palabra").val().trim();					//document.getElementById("palabra").value.trim();
		let vConcepto=$("#concepto").val().trim();					//document.getElementById("concepto").value.trim();
		let palabras=padre.find("dt");
		let conceptos=padre.find("dd");			
		let indice=0;
		let indice2=0;
		let inexistente=true;
		let aux=$(palabras).eq(1);
			...
			while (inexistente && indice < palabras.length){
				...
				if (vPalabra == palabraRepetida){
					while (inexistente && indice2 < conceptos.length){
						...
						let conceptoRepetido=conceptoRepe.val();
						
						if(vConcepto==conceptoRepetido){
							inexistente=false;
						}
						else if(vConcepto!=conceptoRepetido){
							palabraRepe.append("<dd>"+vConcepto+"</dd>").nextUntil("dd");
							inexistente=false;
						}
						indice2+=1;
					}
		...
		if(vMarca!="" && vModelo!="" && vPrecio!=""){				
			...
			while (inexistente && indice < lineas.length){
				...
				if (celdas.eq(0).text()==vMarca && celdas.eq(1).text()==vModelo && celdas.eq(2).text()!=vPrecio){
					...
					inexistente=false;
					$(lin).eq(indice).before("<tr><td>"+vMarca+"</td><td>"+vModelo+"</td><td>"+vPrecio+"</td></tr>");
				}
				else if (marcaActual < marcaLinea){
					inexistente=false;
