// añadir elementos a la tabla sin repeticiones de nombre
...
function anyadirElemento(){
	let valorNombre=document.getElementById("nombre").value.trim();
	let valorFamilia=document.getElementById("familia").value.trim();
	...
	if (valorNombre!="" && valorFamilia!="" && valorEjemplares!=""){
		...
		while (ausente && indice < lineas.length){
			...
			if (valorNombre.toLowerCase()==celdas.item(0).textContent.toLowerCase())
				ausente=false;
			indice+=1;
		}
		if (ausente){
